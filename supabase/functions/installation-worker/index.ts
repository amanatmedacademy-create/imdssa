...
type TenantBinding = { external_tenant_id: string; status: string };
type PermissionRow = { permission: string };
type ProductResult = { healthy?: boolean; status?: string; checks?: unknown; error?: unknown; [key: string]: unknown };
...
function endpointFor(operation: InstallationJob['operation']): { method: 'GET' | 'POST'; path: string } {
  ...
  const action = operation === 'install' ? 'provision' : operation;
  return { method: 'POST', path: `/internal/platform/modules/${action}` };
}
...
async function loadContext(settings: Env, job: InstallationJob) {
  ...
  let bindings = await rest<TenantBinding[]>(settings, 'product_tenant_bindings',
    `select=external_tenant_id,status&organization_id=eq.${installation.organization_id}&product_id=eq.${ownerProduct.id}&environment=eq.production&status=eq.active&limit=1`);
  if (!bindings[0]) {
    bindings = await rest<TenantBinding[]>(settings, 'product_tenant_bindings',
      `select=external_tenant_id,status&organization_id=eq.${installation.organization_id}&product_id=eq.${hostProduct.id}&environment=eq.production&status=eq.active&limit=1`);
  }
...
async function complete(
  ...
  const health = succeeded && result.healthy === true
    ? 'healthy'
    : succeeded && job.operation === 'suspend'
      ? 'healthy'
      : succeeded && job.operation === 'uninstall'
        ? 'unknown'
        : succeeded
          ? 'degraded'
          : 'failed';
  await rpc(settings, 'complete_installation_job', {
    job_id_value: job.id,
    worker_id_value: 'crm-kanban-installation-worker',
    succeeded_value: succeeded,
    health_status_value: health,
    current_step_value: succeeded ? 'completed' : 'product_command_failed',
    result_value: result,
    error_value: error,
  });
}
