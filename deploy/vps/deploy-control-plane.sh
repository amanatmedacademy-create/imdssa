...
if [ ! -f "$ENV_DIR/postgres.env" ]; then echo "Missing $ENV_DIR/postgres.env. Run bootstrap-postgres.sh first." >&2; exit 1; fi
chown root:imdssa "$ENV_DIR/postgres.env"; chmod 0640 "$ENV_DIR/postgres.env"
if [ ! -f "$ENV_DIR/telegram.env" ]; then cat > "$ENV_DIR/telegram.env" <<'EOF'
TELEGRAM_BOT_TOKEN=
TELEGRAM_CHAT_ID=
EOF
fi
chown root:imdssa "$ENV_DIR/telegram.env"; chmod 0640 "$ENV_DIR/telegram.env"
...
chown root:imdssa "$ENV_DIR/api.env"; chmod 0640 "$ENV_DIR/api.env"
cat > /etc/systemd/system/imds-super-admin-api.service <<'EOF'
[Unit]
Description=IMDS Control Center API
...
source "$ENV_DIR/postgres.env"; export DATABASE_URL
if ! sudo -u postgres psql --dbname=imdssa --tuples-only --no-align --command="select count(*) from app.platform_users" | grep -Eq '^[1-9][0-9]*$'; then
  ...
  umask 077; cat > /root/imdssa-initial-admin.txt <<EOF
IMDS Control Center initial credentials
...
sudo -u postgres psql --dbname=imdssa --tuples-only --no-align --command="select to_regclass('app.product_plans') is not null and to_regclass('app.product_limit_catalog') is not null and to_regclass('app.product_module_prices') is not null and to_regclass('app.product_plan_revisions') is not null" | grep -q '^t$'
...
echo "IMDS Control Center deployed on port 8080"
