...
const tabTitles: Record<Tab, string> = {
  overview: 'Обзор платформы', organizations: 'Организации', products: 'Продукты', modules: 'Модули', installations: 'Установки модулей', sync: 'Синхронизация продуктов', realtime: 'События в реальном времени',
};
...
export function VpsApp() {
  ...
  {tab === 'installations' && <><section className="vps-card"><div className="vps-card-head"><div><span>PROVISIONING</span><h2>Установить модуль</h2></div></div><form className="vps-form-grid" onSubmit={installModule}><label>Организация<select required value={installForm.organizationId} onChange={(e) => setInstallForm({ ...installForm, organizationId: e.target.value })}><option value="">Выберите</option>{organizations.map((x) => <option key={x.id} value={x.id}>{x.name}</option>)}</select></label><label>Host product<select required value={installForm.hostProductId} onChange={(e) => setInstallForm({ organizationId: installForm.organizationId, hostProductId: e.target.value, moduleId: '' })}><option value="">Выберите</option>{products.map((x) => <option key={x.id} value={x.id}>{x.name}</option>)}</select></label><label>Модуль<select required value={installForm.moduleId} onChange={(e) => setInstallForm({ ...installForm, moduleId: e.target.value })}><option value="">Выберите</option>{availableModules.map((x) => <option key={x.id} value={x.id}>{x.name}</option>)}</select></label><button className="vps-action" disabled={busy}>Установить</button></form><p className="vps-note">Desired state записывается сразу. ACTIVE считается фактически применённым только после ACK от продукта.</p></section><section className="vps-card"><div className="vps-card-head"><div><span>RUNTIME</span><h2>Установки модулей</h2></div></div>{installations.length === 0 ? <EmptyState title="Активных установок нет" text="После установки здесь появятся desired, actual и sync status." /> : <div className="vps-table-wrap"><table><thead><tr><th>Организация / модуль</th><th>Host product</th><th>Desired</th><th>Actual</th><th>Sync</th><th>Revision</th><th>Действие</th></tr></thead><tbody>{installations.map((x) => <tr key={x.id}><td><strong>{x.organization_name}</strong><small>{x.module_name}</small></td><td>{x.host_product_name}</td><td><Status value={x.status} /></td><td>{x.actual_enabled == null ? '—' : <Status value={x.actual_enabled ? 'active' : 'disabled'} />}</td><td><Status value={x.sync_status || 'pending'} /></td><td>{x.last_applied_revision ?? 0}</td><td><button className="vps-mini" disabled={busy} onClick={() => void setInstallationStatus(x.id, x.status === 'active' ? 'suspended' : 'active')}>{x.status === 'active' ? 'Отключить' : 'Включить'}</button></td></tr>)}</tbody></table></div>}</section></>}

  {tab === 'sync' && <section className="vps-card"><div className="vps-card-head"><div><span>CONTROL COMMANDS</span><h2>Desired → Actual</h2></div><Status value={(overview?.sync_pending ?? 0) === 0 ? 'synced' : 'pending'} /></div>{commands.length === 0 ? <EmptyState title="Команд пока нет" text="После изменения продукта или модуля здесь появится durable command и подтверждение продукта." /> : <div className="vps-table-wrap"><table><thead><tr><th>Организация</th><th>Продукт</th><th>Команда</th><th>Revision</th><th>Статус</th><th>Попытки</th><th>Ошибка</th></tr></thead><tbody>{commands.map((x) => <tr key={x.id}><td>{x.organization_name}</td><td><strong>{x.product_name}</strong><small>{x.product_code}</small></td><td>{x.command_type}</td><td>{x.desired_revision}</td><td><Status value={x.status} /></td><td>{x.attempts}</td><td>{x.last_error ? <small className="vps-inline-error">{x.last_error}</small> : '—'}</td></tr>)}</tbody></table></div>}</section>}
