...
type Installation = { id: string; organization_id: string; module_id: string; host_product_id: string; organization_name: string; module_code: string; module_name: string; host_product_name: string; status: string; health: string; version: string | null; updated_at: string };
type OrganizationProduct = { organization_id: string; product_id: string; organization_name: string; product_name: string; product_code: string; status: string };
...
const tabTitles: Record<Tab, string> = {
  overview: 'Обзор платформы', organizations: 'Организации', products: 'Продукты', modules: 'Модули', installations: 'Установки модулей', realtime: 'События в реальном времени',
};
...
function Login({ onReady }: { onReady: (user: User) => void }) {
  ...
  return <main className="vps-login"><form className="vps-login-card" onSubmit={submit}><div className="vps-brand"><b>IMDS</b><span>Super Admin</span></div><div className="vps-login-copy"><span>CONTROL PLANE</span><h1>Вход в платформу</h1><p>Локальный VPS · PostgreSQL · realtime</p></div><label>Email<input value={email} onChange={(e) => setEmail(e.target.value)} type="email" autoComplete="username" required /></label><label>Пароль<input value={password} onChange={(e) => setPassword(e.target.value)} type="password" autoComplete="current-password" required /></label>{error && <div className="vps-error">{error}</div>}<button className="vps-primary" type="submit">Войти</button></form></main>;
}

function Status({ value }: { value: string }) { const normalized = value || 'unknown'; return <span className={`vps-status ${normalized}`}>{normalized.toUpperCase()}</span>; }
...
export function VpsApp() {
  ...
  const [entitlementForm, setEntitlementForm] = useState({ organizationId: '', productId: '' });
  const [installForm, setInstallForm] = useState({ organizationId: '', hostProductId: '', moduleId: '' });

  const refresh = useCallback(async () => {
    try {
      const [o, org, prod, mod, inst, entitlements] = await Promise.all([
        api<Overview>('/api/v1/overview'), api<{ items: Organization[] }>('/api/v1/organizations'), api<{ items: Product[] }>('/api/v1/products'), api<{ items: Module[] }>('/api/v1/modules'), api<{ items: Installation[] }>('/api/v1/installations'), api<{ items: OrganizationProduct[] }>('/api/v1/organization-products'),
      ]);
      setOverview(o); setOrganizations(org.items); setProducts(prod.items); setModules(mod.items); setInstallations(inst.items); setOrganizationProducts(entitlements.items); setError('');
  ...
  const enableProduct = async (event: FormEvent) => { event.preventDefault(); setBusy(true); try { await api('/api/v1/organization-products', { method: 'POST', body: JSON.stringify({ ...entitlementForm, status: 'active' }) }); await refresh(); } catch (e) { setError(e instanceof Error ? e.message : 'Ошибка'); } finally { setBusy(false); } };
  ...
  {tab === 'products' && <><section className="vps-card"><div className="vps-card-head"><div><span>PRODUCT ACCESS</span><h2>Выдать продукт организации</h2></div></div><form className="vps-form-grid compact" onSubmit={enableProduct}><label>Организация<select required value={entitlementForm.organizationId} onChange={(e) => setEntitlementForm({ ...entitlementForm, organizationId: e.target.value })}><option value="">Выберите</option>{organizations.map((x) => <option key={x.id} value={x.id}>{x.name}</option>)}</select></label><label>Продукт<select required value={entitlementForm.productId} onChange={(e) => setEntitlementForm({ ...entitlementForm, productId: e.target.value })}><option value="">Выберите</option>{products.map((x) => <option key={x.id} value={x.id}>{x.name}</option>)}</select></label><button className="vps-action" disabled={busy}>Включить продукт</button></form></section><section className="vps-card"><div className="vps-card-head"><div><span>PRODUCT REGISTRY</span><h2>Продукты IMDS</h2></div></div>{products.length === 0 ? <EmptyState title="Нет зарегистрированных продуктов" text="Control plane не создаёт демонстрационные записи." /> : <div className="vps-table-wrap"><table><thead><tr><th>Продукт</th><th>Организации</th><th>Статус</th><th>Health</th></tr></thead><tbody>{products.map((x) => <tr key={x.id}><td><strong>{x.name}</strong><small>{x.code}</small></td><td>{x.tenants}</td><td><Status value={x.status} /></td><td><Status value={x.last_health} /></td></tr>)}</tbody></table></div>}</section>{organizationProducts.length > 0 && <section className="vps-card"><div className="vps-card-head"><div><span>ENTITLEMENTS</span><h2>Выданные продукты</h2></div></div><div className="vps-table-wrap"><table><thead><tr><th>Организация</th><th>Продукт</th><th>Статус</th></tr></thead><tbody>{organizationProducts.map((x) => <tr key={`${x.organization_id}:${x.product_id}`}><td>{x.organization_name}</td><td><strong>{x.product_name}</strong><small>{x.product_code}</small></td><td><Status value={x.status} /></td></tr>)}</tbody></table></div></section>}</>}

  {tab === 'modules' && <section className="vps-card"><div className="vps-card-head"><div><span>MODULE CATALOG</span><h2>Модули</h2></div></div>{modules.length === 0 ? <EmptyState title="Модули не зарегистрированы" text="Каталог формируется только из реально существующих модулей продуктов." /> : <div className="vps-table-wrap"><table><thead><tr><th>Модуль</th><th>Категория</th><th>Продукт</th><th>Статус</th></tr></thead><tbody>{modules.map((x) => <tr key={x.id}><td><strong>{x.name}</strong><small>{x.code}</small></td><td>{x.category}</td><td>{x.owner_product_name || '—'}</td><td><Status value={x.status} /></td></tr>)}</tbody></table></div>}</section>}

  {tab === 'installations' && <><section className="vps-card"><div className="vps-card-head"><div><span>PROVISIONING</span><h2>Установить модуль</h2></div></div><form className="vps-form-grid" onSubmit={installModule}><label>Организация<select required value={installForm.organizationId} onChange={(e) => setInstallForm({ ...installForm, organizationId: e.target.value })}><option value="">Выберите</option>{organizations.map((x) => <option key={x.id} value={x.id}>{x.name}</option>)}</select></label><label>Host product<select required value={installForm.hostProductId} onChange={(e) => setInstallForm({ organizationId: installForm.organizationId, hostProductId: e.target.value, moduleId: '' })}><option value="">Выберите</option>{products.map((x) => <option key={x.id} value={x.id}>{x.name}</option>)}</select></label><label>Модуль<select required value={installForm.moduleId} onChange={(e) => setInstallForm({ ...installForm, moduleId: e.target.value })}><option value="">Выберите</option>{availableModules.map((x) => <option key={x.id} value={x.id}>{x.name}</option>)}</select></label><button className="vps-action" disabled={busy}>Установить</button></form><p className="vps-note">Установка разрешена только если выбранный продукт уже выдан организации.</p></section><section className="vps-card"><div className="vps-card-head"><div><span>RUNTIME</span><h2>Установки модулей</h2></div></div>{installations.length === 0 ? <EmptyState title="Активных установок нет" text="После установки здесь появится runtime status. Все изменения пишутся в audit и realtime." /> : <div className="vps-table-wrap"><table><thead><tr><th>Организация / модуль</th><th>Host product</th><th>Статус</th><th>Health</th><th>Действие</th></tr></thead><tbody>{installations.map((x) => <tr key={x.id}><td><strong>{x.organization_name}</strong><small>{x.module_name}</small></td><td>{x.host_product_name}</td><td><Status value={x.status} /></td><td><Status value={x.health} /></td><td><button className="vps-mini" disabled={busy} onClick={() => void setInstallationStatus(x.id, x.status === 'active' ? 'suspended' : 'active')}>{x.status === 'active' ? 'Отключить' : 'Включить'}</button></td></tr>)}</tbody></table></div>}</section></>}
