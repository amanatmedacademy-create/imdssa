...
type Overview = { host: { hostname: string; platform: string; release: string; uptimeSeconds: number; cpuCores: number; cpuPercent: number; loadAverage: number[]; memory: { total: number; used: number; free: number; percent: number }; disk: { total: number; used: number; available: number; percent: number } }; database: { database: string; database_bytes: number; connections: number; active_connections: number }; services: Service[]; deployments: Deployment[]; time: string };
...
export function InfrastructureCenter() {
  ...
  return <>
    ...
    {open && <div className="infra-overlay"><div className="infra-shell">
      ...
      {tab === 'overview' && overview && <><section className="infra-metrics"><article><span>CPU</span><strong>{overview.host.cpuPercent}%</strong><small>{overview.host.cpuCores} ядер</small></article><article><span>RAM</span><strong>{overview.host.memory.percent}%</strong><small>{bytes(overview.host.memory.used)} / {bytes(overview.host.memory.total)}</small></article><article><span>Disk</span><strong>{overview.host.disk.percent}%</strong><small>{bytes(overview.host.disk.used)} / {bytes(overview.host.disk.total)}</small></article><article><span>Services</span><strong>{healthyServices}/{services.length}</strong><small>active</small></article><article><span>DB connections</span><strong>{overview.database.connections}</strong><small>{overview.database.active_connections} active</small></article><article><span>Uptime</span><strong>{duration(overview.host.uptimeSeconds)}</strong><small>{overview.host.platform} {overview.host.release}</small></article></section><section className="infra-grid"><div className="infra-card span-2"><div className="infra-card-head"><div><span>SERVICES</span><h2>Runtime health</h2></div></div><div className="infra-service-list">{services.map((service) => <div key={service.key}><i className={statusClass(service.active)} /><div><strong>{service.label}</strong><small>{service.unit}</small></div><span>{service.active} / {service.sub}</span></div>)}</div></div><div className="infra-card"><div className="infra-card-head"><div><span>DATABASE</span><h2>{overview.database.database}</h2></div></div><div className="infra-facts"><div><span>Size</span><strong>{bytes(overview.database.database_bytes)}</strong></div><div><span>Connections</span><strong>{overview.database.connections}</strong></div><div><span>Active</span><strong>{overview.database.active_connections}</strong></div></div></div><div className="infra-card span-3"><div className="infra-card-head"><div><span>DEPLOYMENTS</span><h2>Current releases</h2></div></div><div className="infra-deploy-row">{deployments.map((item) => <div key={item.label}><strong>{item.label}</strong><span>{item.release || '—'}</span><small>{date(item.deployedAt)}</small></div>)}</div></div></section></>}
