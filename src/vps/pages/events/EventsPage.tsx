...
const categoryLabels: Record<FeedCategory, string> = { product: 'Продукт', commercial: 'Коммерция', sync: 'Синхронизация', system: 'Система' };
const sourceLabels: Record<FeedSource, string> = { realtime: 'Realtime', audit: 'Audit', command: 'Control command' };
...
export function EventsPage({ user, realtimeEvents, commands, organizations, products, realtimeState }: Props) {
  ...
  useEffect(() => {
    ...
    api<{ items: AuditLog[] }>('/api/v1/audit')
      ...
      .catch((error) => { if (!cancelled) setAuditError(error instanceof Error ? error.message : 'Ошибка Audit API'); });
  ...
  const feed = useMemo<FeedItem[]>(() => {
    ...
    const live: FeedItem[] = realtimeEvents.map((item, index) => {
      ...
      const payloadData = objectValue(item.payload, ['data']) ? item.payload : item.payload;
      ...
      const organizationIdValue = item.organization_id || organizationFromPayload;
      const productIdValue = item.product_id || productFromPayload;
      return {
  ...
  return <section className="events-page">
    <div className="events-kpis">
      <article><span>Realtime</span><strong>{liveCount}</strong><small>с начала сессии</small></article>
      <article><span>Audit</span><strong>{audit.length}</strong><small>{canViewAudit ? 'последние 100' : 'ограничено ролью'}</small></article>
      <article><span>Коммерческие</span><strong>{commercialCount}</strong><small>billing / subscriptions</small></article>
      <article className={failedCommands ? 'danger' : ''}><span>Failed commands</span><strong>{failedCommands}</strong><small>control plane</small></article>
      <article><span>Канал</span><strong className={`events-live-state ${realtimeState}`}>{realtimeState === 'online' ? 'Online' : realtimeState === 'connecting' ? 'Connecting' : 'Offline'}</strong><small>PostgreSQL LISTEN / NOTIFY</small></article>
    </div>

    <div className="events-toolbar">
      <label className="events-search"><Search size={16}/><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Тип, организация, продукт, payload…"/></label>
      <label><SlidersHorizontal size={15}/><select value={category} onChange={(event) => setCategory(event.target.value as 'all' | FeedCategory)}><option value="all">Все контуры</option>{Object.entries(categoryLabels).map(([id, label]) => <option key={id} value={id}>{label}</option>)}</select></label>
    ...
    {auditError && <div className="events-note"><UserRoundCog size={16}/><span>Audit history недоступна: {auditError}. Realtime-поток продолжает работать отдельно.</span></div>}

    <div className="events-workspace">
      <div className="events-list-panel">
        <div className="events-panel-head"><div><span>EVENT STREAM</span><h2>События</h2></div><small>{filtered.length} записей</small></div>
        {!filtered.length ? <EmptyState title="Событий нет" text="Новые изменения появятся здесь через realtime-канал."/> : <div className="events-list">{filtered.map((item) => <button key={item.key} type="button" className={selectedKey === item.key ? 'active' : ''} onClick={() => setSelectedKey(item.key)}>
      ...
      <div className="events-detail-panel">
        {!selected ? <EmptyState title="Выберите событие" text="Здесь будут источник, контекст и payload."/> : <>
          <div className="events-detail-head"><div><span>{sourceLabels[selected.source].toUpperCase()}</span><h2>{selected.title}</h2><p>{selected.type}</p></div><span className={`events-category ${selected.category}`}>{categoryLabels[selected.category]}</span></div>
          ...
          <div className="events-payload">
            <div className="events-section-head"><div><Braces size={16}/><span><strong>Payload</strong><small>Фактические данные события без преобразования бизнес-состояния</small></span></div></div>
            <pre>{safeJson(selected.payload)}</pre>
          </div>
          <div className="events-source-note"><Activity size={16}/><div><strong>Потоки не смешивают источники истины.</strong><p>Realtime отражает изменения PostgreSQL, Audit — действия пользователей, Control command — доставку desired state в продукт.</p></div></div>
        </>}
      </div>
    </div>
  </section>;
