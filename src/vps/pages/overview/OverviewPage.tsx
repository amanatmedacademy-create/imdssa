...
export function OverviewPage({ overview, organizations, products, installations, commands, realtimeState, onRefresh, onNavigate }: Props) {
  ...
  const platformState = unhealthyProducts > 0 || failedCommands.length > 0
    ...
    : degradedProducts > 0 || syncPending > 0 || realtimeState !== 'online'
      ? { tone: 'warn', title: 'Есть отклонения', text: 'Продуктовый контур работает, но есть незавершённая синхронизация или деградация.', icon: Activity }
      : { tone: 'good', title: 'Продуктовый контур стабилен', text: 'Организации, продукты, модули и синхронизация работают без критичных отклонений.', icon: ShieldCheck };
  ...
  return <section className="overview-page">
    ...
    <div className="overview-grid">
      ...
      <aside className="overview-panel overview-panel-control"><div className="overview-panel-head"><div><span>CONTROL PLANE</span><h2>Операционный контур</h2><p>Ключевые показатели управления продуктами.</p></div></div><div className="overview-facts"><div><span><CheckCircle2 size={15} />Активные установки</span><strong>{overview?.installations ?? activeInstallations}</strong></div><div><span><Users size={15} />Пользователи платформы</span><strong>{overview?.platform_users ?? 0}</strong></div><div><span><Workflow size={15} />Команды в работе</span><strong>{pendingCommands.length}</strong></div><div className={failedCommands.length ? 'is-danger' : ''}><span><CircleAlert size={15} />Ошибки команд</span><strong>{failedCommands.length}</strong></div></div><div className="overview-control-note"><span>Источник истины</span><strong>Control Center · PostgreSQL</strong><small>Entitlement и коммерческое состояние продуктов управляются центральным контуром.</small></div></aside>
    </div>
